...
<xsl:stylesheet version="1.0"
 ...
 <xsl:attribute name="title">بحث عن الناشر</xsl:attribute>
 <xsl:value-of select="."/>
 </a>
 <xsl:if test="position() != last()">
 <xsl:text>, </xsl:text>
 ...
 <span class="label">
 <xsl:choose>
 <xsl:when test="@ind2=1">الإنتاج: </xsl:when>
 <xsl:when test="@ind2=2">التوزيع: </xsl:when>
 <xsl:when test="@ind2=3">صناعة: </xsl:when>
 <xsl:when test="@ind2=4">
 <xsl:choose>
 <xsl:when test="substring(marc:subfield[@code='d'],1,1)='C'">تاريخ حقوق النشر: </xsl:when>
 <xsl:when test="substring(marc:subfield[@code='d'],1,1)='P'">تاريخ الحماية: </xsl:when>
 <xsl:otherwise>بيانات حقوق الطبع والنشر / تاريخ الحماية: </xsl:otherwise>
 </xsl:choose>
 </xsl:when>
 <xsl:otherwise>النشر: </xsl:otherwise>
 </xsl:choose>
 </span>
 <span>
 <xsl:call-template name="addClassRtl" />
 ...
 <span class="label">معلومات الطباعة و/أو النشر المكتوبة كما هي موجودة في بيانات النسخ: </span>
 ...
 <span class="label">طباعة و/أو نشر المعلومات المكتوبة كما هي موجودة في المصدر الرئيسي للمعلومات: </span>
 ...
 <span class="label">مرتبط مع: </span>
 <span class="value">
 <xsl:call-template name="addClassRtl" />
