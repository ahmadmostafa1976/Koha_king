...
-->
 ...
 <xsl:text>مباشر</xsl:text>
 </xsl:attribute>
 </xsl:when>
 <xsl:when test="$controlField008-06='i'">
 <xsl:attribute name="geographicSubdivision">
 <xsl:text>غير مباشر</xsl:text>
 </xsl:attribute>
 </xsl:when>
 <xsl:when test="$controlField008-06='n'">
 <xsl:attribute name="geographicSubdivision">
 <xsl:text>غير قابل للتطبيق</xsl:text>
 </xsl:attribute>
 </xsl:when>
 </xsl:choose>
 ...
 <mads:recordInfo>
 <mads:recordOrigin>محوّل من MARCXML إلى MADS الإصدار 2.0 (مراجعة 2.10)</mads:recordOrigin>
 <!-- <xsl:apply-templates select="marc:datafield[@tag=024]"/> -->
 ...
 <mads:descriptionStandard>
 <xsl:text>aacr2 متوافق مع</xsl:text>
 </mads:descriptionStandard>
 </xsl:if>
 <xsl:if test="substring(.,11,1)='z'">
 <mads:descriptionStandard>
 <xsl:text>قواعد أخرى</xsl:text>
 </mads:descriptionStandard>
 </xsl:if>
 </xsl:for-each>
 </mads:recordInfo>
...
-->
 ...
 <xsl:text>-</xsl:text>
 <xsl:call-template name="subfieldSelect">
 <xsl:with-param name="codes">شارع</xsl:with-param>
 </xsl:call-template>
 </mads:fieldOfActivity>
 </xsl:template>
 ...
 <xsl:attribute name="type">غير عام</xsl:attribute>
 </xsl:when>
 <xsl:when test="@tag=670">
 <xsl:attribute name="type">المصدر</xsl:attribute>
 </xsl:when>
 <xsl:when test="@tag=675">
 <xsl:attribute name="type">لم يتم العثور عليه</xsl:attribute>
 </xsl:when>
 <xsl:when test="@tag=678">
 <xsl:attribute name="type">تاريخ</xsl:attribute>
 </xsl:when>
 <xsl:when test="@tag=681">
 <xsl:attribute name="type">مثال الموضوع</xsl:attribute>
 </xsl:when>
 <xsl:when test="@tag=682">
 <xsl:attribute name="type">معلومات الرأس المحذوفة</xsl:attribute>
 </xsl:when>
 <xsl:when test="@tag=688">
 <xsl:attribute name="type">سِجل التطبيق</xsl:attribute>
 </xsl:when>
 </xsl:choose>
 <xsl:call-template name="chopPunctuation">
 <xsl:with-param name="chopString">
 ...
 <xsl:attribute name="type">سابق</xsl:attribute>
 </xsl:if>
 <xsl:if test="substring(marc:subfield[@code='w'],1,1)='b'">
 <xsl:attribute name="type">لاحقا</xsl:attribute>
 </xsl:if>
 <xsl:if test="substring(marc:subfield[@code='w'],1,1)='t'">
 <xsl:attribute name="type">المؤسسة الرئيسية</xsl:attribute>
 </xsl:if>
 <xsl:if test="substring(marc:subfield[@code='w'],1,1)='g'">
 <xsl:attribute name="type">أوسع</xsl:attribute>
 </xsl:if>
 <xsl:if test="substring(marc:subfield[@code='w'],1,1)='h'">
 <xsl:attribute name="type">أضيق</xsl:attribute>
 </xsl:if>
 <xsl:if test="substring(marc:subfield[@code='w'],1,1)='r'">
 <xsl:attribute name="type">آخر</xsl:attribute>
 </xsl:if>
 ...
 <xsl:otherwise>
 <!-- 7xx -->
 <xsl:attribute name="type">مساو</xsl:attribute>
 </xsl:otherwise>
 </xsl:choose>
 <xsl:apply-templates select="marc:subfield[@code='i']"/>
 </xsl:template>
 ...
 <xsl:attribute name="type">حروف مختصرة</xsl:attribute>
 </xsl:if>
 ...
 <xsl:attribute name="authority">تخبأة</xsl:attribute>
 </xsl:when>
 <xsl:when
 ...
 <xsl:attribute name="authority">شبكية</xsl:attribute>
 </xsl:when>
 <xsl:when
